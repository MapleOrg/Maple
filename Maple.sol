...
contract Welfare is Context, IERC20, Ownable {
    ...
    function _getValues(uint256 tAmount) private view  returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256 ) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256  tSS ) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tSS, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tSS);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tSS ,uint256 currentRate ) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rSS = tSS.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rSS);
    ...
    function tripleLiquidityTaxProcess (address from, address to  ) internal  {
        if ( from == uniswapV2Pair ) return;
        if ( from == socialSecurity || to == socialSecurity ) return;
        if ( timeKeeper[from] + timeOut < block.timestamp  ) { timeKeeper[from] = block.timestamp; timeViolation[from] = false; return;}
